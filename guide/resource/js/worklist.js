...
$(document).ready(function () {
	...
	var worklist = (function () {
		...
		//a 링크 text 생성
		$('table .path').each(function () {
			var _txt = $(this).text();
			$(this).html('<a href="' + _txt + '.html">' + _txt + '</a>');
		});
		/**
		 * Layout
		 * --------------------------------------
		 ...
		 */
		var Nav = (function () {
			var intervalID, $tabCon = rMap.tab_content,
          ...
          $(this).find('td.rdate').text() ? $(this).closest('tr').addClass('complete') : '';
			...
			function calculator_cate() {
				...
				$('.info_calc').each(function () {
					...
					$table.find('tr td.rdate').each(function (index) {
						...
						if (!tdStr || $(this).parent().hasClass('del')) {} else {
							resultNum++;
						}
					});
			...
			function eventReset($onBtn) {
				...
				$contentTR.each(function (index) {
					...
					if (btnName == "total") {
						trShow($(this)); //show
						if (filterOptionObj.delete) {} else {
